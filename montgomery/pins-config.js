...
jQuery(function () {
  ...
  if(pins_len > 0) {
    ...
    var tsvg_obj = document.getElementById("uswijspins");
    ...
    for (var i = 0; i < pins_len; i++) {
      if (pins_config.pins[i].shape === "circle") {
        ...
        svg_circle.setAttributeNS(null, "id", "uswijspins_" + i);
        tsvg_obj.appendChild(svg_circle);
        uswijsAddEvent(i);
      }
        ...
        svg_rect.setAttributeNS(null, "id", "uswijspins_" + i);
        tsvg_obj.appendChild(svg_rect);
        uswijsAddEvent(i);
      }
    }
...
function uswijsAddEvent(id) {
  var obj = jQuery("#uswijspins_" + id);
  if(pins_config.pins[id].active === true){
    obj.attr({"cursor": "pointer"});
    obj.hover(function () {
      jQuery("#uswijstip").show().html(pins_config.pins[id].hover);
